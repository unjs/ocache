...
By default the full query string varies the cache key, so `?color=red` and `?color=red&utm=x` are cached separately and unknown params can bust the cache. Set `variesQuery` to an allowlist of param names so only those affect the key — all other params are ignored. Ignored params are also stripped from the URL the handler receives (like non-`varies` headers), so a handler can never accidentally produce output that depends on a param outside the key. The order of different params is normalized (`?color=red&size=big` and `?size=big&color=red` share one entry), but the order of a repeated param's values is preserved (`?tag=a&tag=b` and `?tag=b&tag=a` are cached separately). Passing an empty array (`variesQuery: []`) varies by nothing — every query shares one entry. If you set a custom `getKey`, it controls the key entirely and `variesQuery` no longer affects it, but non-allowlisted params are still stripped from the URL the handler receives:
