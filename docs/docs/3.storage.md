...
Memory storage honors per-entry TTLs: expired entries return `null` and are cleaned up automatically. A TTL beyond the ~24.8-day `setTimeout` limit is honored too; the timer is chained rather than clamped, so a 30-day `maxAge` does not expire after a millisecond.
