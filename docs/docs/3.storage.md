...
Use this for layered setups such as a fast local tier in front of a shared remote tier. A single string keeps everything in one tier; the default is `"/cache"`. Invalidation and expiration always operate across all tiers — see [Invalidation & Expiration](/docs/invalidation).
