...
- **Cache keys** — built from the handler [name](#cache-key-name), the request [origin](#multi-host-deployments), the URL path, and the method — plus the [query parameters](/docs/query-params) and [headers](#varying-by-request-headers) you choose to vary on.
