...
1. **All durations are in seconds.** `maxAge`, `staleMaxAge`, `maxResolveTime`, and storage TTLs all use seconds, never milliseconds. There is no upper bound: a lifetime past ~24.8 days (the `setTimeout` limit) is timed correctly rather than expiring after a millisecond.
