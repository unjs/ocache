...
Entries live in an in-memory store that is **private to this cached function** unless you pass `storage`. See [Storage](/docs/storage) for backends and sharing, and [Caching HTTP Handlers](/docs/handler) for full response caching.
...
- **Unset** — the stale window has **no time limit**. The last successful value is served for as long as the entry exists, and no storage TTL is written; the entry stays until the backend evicts it. This is the [ISR](/docs/isr) pattern. The built-in memory storage evicts above [10 000 entries or 100 MB](/docs/storage#memory-storage--lru).
...
>
> The standalone [`resolveCacheKeys` / `invalidateCache` / `expireCache`](/docs/invalidation) helpers never see `fn`. When you purge entries with them, always pass the same `name` you cached under.
...
- **A cached function receives no cancellation signal.** Its abandoned resolver keeps running and may still settle, but no caller waits for it, its value is never served, and it cannot overwrite a value a later call cached. A cached **handler** does get a signal — see [the handler guide](/docs/handler#resolution-deadline).
...
What the entry holds depends on one thing: whether your backend declares [`binary`](/docs/storage#the-storage-interface).
...
- **Only a value that _is_ bytes takes this path.** A byte view nested in an object (`{ image, meta }`) is stored as an ordinary property, which a serializing backend turns into `{"0":255,…}`. Use [`serialize`/`transform`](#custom-serialization) for those shapes.
