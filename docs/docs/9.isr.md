...
>
> 1. **Duplicate background refreshes are combined per instance**, not globally. With multiple servers or serverless instances, the origin can receive one refresh from each. If regeneration is expensive, add a distributed lock in your [custom storage](/docs/storage).
