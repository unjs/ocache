...
3. Declare every request header a cached handler reads in `varies` (or use the credential and cookie options), and every query parameter it reads in `allowQuery`.
