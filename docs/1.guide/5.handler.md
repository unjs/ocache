...
> [!TIP]
> **Only headers in `varies` reach the handler.** Every other request header is stripped before the handler runs, so a handler can never accidentally produce output that depends on a header outside the cache key. (Cookies are handled separately — see [Cookies](/guide/cookies).)
...
- [Cache-Control & Eligibility](/guide/cache-control) — `no-store`/`private`, `sendCacheControl`, `shouldCache`.
