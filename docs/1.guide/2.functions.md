...
By default, entries are stored in an in-memory store. See [Storage](/guide/storage) to plug in a persistent backend, and [HTTP Handlers](/handler) for caching full HTTP responses.
...
const getUser = defineCachedFunction(
  ...
  { name: "getUser", maxAge: 60, getKey: (id) => id },
);
