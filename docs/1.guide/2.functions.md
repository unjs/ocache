...
const cached = defineCachedFunction(fn, {
  ...
  resolverTimeout: 30_000, // ms a single resolution may take (default: 30s)
...
`resolverTimeout` puts a deadline on it, in **milliseconds**, defaulting to **30 seconds**. It covers the whole shared resolution — the resolver plus the `getMaxAge` and `serialize` hooks that run with it, which is where a never-ending response body would be drained.

```ts
const getReport = defineCachedFunction(fetchFromUpstream, {
  name: "report",
  maxAge: 300,
  resolverTimeout: 5000, // give up on a resolution after 5s
});
```
...
Pass `Infinity` (or `0`) to disable the deadline and wait forever. Raise it for resolvers that are legitimately slow — the cost of a deadline is that a resolution which would have completed just past it fails instead.
