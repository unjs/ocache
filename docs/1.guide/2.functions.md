...
`maxResolveTime` puts a deadline on it, in **seconds** (like `maxAge` and every other time value here), defaulting to **30**. It covers the whole shared resolution — the resolver plus the `getMaxAge` and `serialize` hooks that run with it, which is where a never-ending response body would be drained.
...
Fractions are allowed, so `maxResolveTime: 0.5` is half a second. Pass `Infinity` (or `0`) to disable the deadline and wait forever. Raise it for resolvers that are legitimately slow — the cost of a deadline is that a resolution which would have completed just past it fails instead.
