...
> [!TIP]
> **Ignored params never reach the handler.** Just like non-allowlisted cookies and [undeclared headers](/guide/handler#headers-the-handler-cant-see), params outside the allowlist are stripped from the URL the handler receives — so a handler can never accidentally produce output that depends on a param outside the cache key.
