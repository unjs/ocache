...
- Honors explicit `Cache-Control: no-store` / `private` on the response — those are never cached (via `validate`), though still returned to the caller. They also opt out of in-flight request coalescing (via the `isShareable` hook): concurrent callers coalesced onto the same request re-resolve independently so one caller's private response never bleeds to another. Tradeoffs, all inherent to the fact that shareability is only knowable from a resolved value (a coalesced caller must await the leader before it can learn the response is non-shareable): (1) opting out of coalescing disables request deduplication for those responses — under a concurrency burst a `no-store`/`private` endpoint re-invokes the handler once per coalesced caller (origin load scales with fan-out) instead of sharing a single resolution; (2) a coalesced caller returns in ~2× the resolution time (it waits out the leader, then re-resolves its own value — resolving in parallel up front is not possible without re-invoking the handler on the common shareable path too); (3) if a coalesced caller's independent re-resolve throws, the error propagates — it is never given the leader's value as a fallback, since sharing a private response is the exact bleed this prevents
