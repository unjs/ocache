...
- Emits a `Cache-Tag` response header from `opts.tags` (opt-in, ergonomic sugar for Cloudflare Workers Cache / generic tag-based purge): when set to a non-empty list, ocache trims + dedupes the tags and joins them with `", "` into the `Cache-Tag` header. Mirrors the preserve-if-present behavior of `etag`/`last-modified`/`cache-control` — an explicit `Cache-Tag` set by the handler is left untouched and not overwritten. The header is **purely advisory**: ocache does no tag-based indexing or invalidation of its own storage; the consumer (e.g. Cloudflare Workers Cache) is expected to index the tags and serve `ctx.cache.purge({ tags: [...] })`. Omit the option (or pass an empty / whitespace-only list) to set no `Cache-Tag` at all. Persisted in the `ResponseCacheEntry` and replayed verbatim on a cache hit
