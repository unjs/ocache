...
- `maxAge <= 0` means "immediately stale": with `swr` and a positive `staleMaxAge` the stale value is served (bounded by that window) while refreshing in the background; with `staleMaxAge: 0` or `swr: false` every access blocks on the resolver. Entries whose servable window is zero (no freshness, no stale window) are never written to storage — a previously stored entry is evicted instead (**behavior change** vs. pre-`honorCacheControl` releases, where `maxAge: 0` + `swr` entries were stored forever and served stale without bound)
...
- `honorCacheControl` option (opt-in, default off) — honor the freshness directives on the handler (upstream) response's `Cache-Control` (`s-maxage` preferred over `max-age` → `maxAge`, `stale-while-revalidate` → `staleMaxAge`; quoted values like `max-age="60"` are accepted) when deriving the per-entry TTL. An upstream directive wins for its field (no clamping — it can shorten or extend); absent fields fall back to the user's `getMaxAge` (the hook is skipped entirely when upstream defines both fields), then the static `maxAge`/`staleMaxAge`. Serving stale requires explicit permission (RFC 9111 §4.2.4): an upstream freshness lifetime without a `stale-while-revalidate` directive — or an explicit `must-revalidate`/`proxy-revalidate`, which wins even over `stale-while-revalidate` (`s-maxage` implies `proxy-revalidate` per §5.2.2.10) — forces `staleMaxAge` to `0` (once stale — immediately for `max-age=0`/`s-maxage=0` — revalidation is blocking, never served stale). A valueless `stale-while-revalidate` still grants stale serving; its window falls back to `getMaxAge`/static `staleMaxAge`. Only a handler-set `Cache-Control` counts as upstream: the resolver parses it _before_ the synthesized header is added and stashes it in a per-handler `WeakMap` keyed by response value, so the header synthesized from static options is never parsed back. `no-cache` is honored by never caching the response (rejected in `validate`, like `no-store`) — the handler runs on every request. `no-store` / `private` handling is independent (always honored)
