...
- `serialize(entry, { args, background })` option — write-side counterpart of `transform`: runs once right after the resolver (and after `getMaxAge`, so that hook still sees the raw value) and returns the value to persist. `transform` deserializes it back on read. Use for resolver outputs a storage backend can't persist as-is (raw `ReadableStream`, class instances). It runs **exactly once per resolution** — shared across concurrent, deduplicated calls (every caller observes the serialized value), so consuming a one-shot source like a stream is safe. A throwing `serialize` fails the call and evicts, like a rejected resolver. Both `getMaxAge` and `serialize` are folded into the shared in-flight (`pending`) promise so they never run more than once. `ctx.background` is `true` when the resolution is an SWR background refresh the caller was **not** blocked on (it was already served the stale value), `false` for a foreground miss the caller is waiting on — `http.ts` uses this to stream only foreground misses
...
- `stream: true` opt-in streams a cache MISS to the client while a copy is cached in the background, instead of buffering the whole body first (better time-to-first-byte for large/proxied bodies). Coordinated via a per-event `WeakMap<HTTPEvent, StreamDeferred>` handshake: the outer wrapper creates a deferred, then races `_cachedHandler` against `deferred.promise`. On a foreground MISS the internal `serialize` hook `tee()`s the resolved body — one branch resolves the deferred (client) before it buffers the other (storage), so the deferred wins the race and the wrapper answers with a live stream while the store completes (kept alive by `event.req.waitUntil`, forwarded across request narrowing). The streamed response carries `cache-control`/`last-modified`/`Vary` and stripped cookies but **no body-hash `etag`** (can't hash without buffering) and reports `x-cache: MISS`; subsequent HITs serve the buffered entry (with etag) normally. **Only foreground misses stream** — the tee is gated on `!ctx.background`, so an SWR background refresh never hijacks a stale hit into a foreground stream (the caller keeps getting its instant `STALE` copy). Concurrent cold MISSes aren't co-streamed (a `ReadableStream` has one consumer); coalesced peers get the buffered copy. Bodyless responses, bypassed methods, and HITs fall through to the buffered path. Tradeoffs (mid-stream failure can't be un-sent as a `5xx`; cache written after flush; transient memory under a slow client) are documented in README
