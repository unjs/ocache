...
- **`StorageInterface.binary` decides the stored body form and the charge factor together.** A backend declares whether it returns byte views intact; `http/entry.ts` reads that one flag to choose between a `Uint8Array` body and base64, and to divide `maxEntryBytes` by `2` rather than `8/3`. Never infer the capability from a value, and never let the two decisions consult different sources. The read path is the exception in the other direction: `deserializeEntry` accepts both stored forms whatever the flag currently says, because entries outlive a change to it, and `validateEntry` rejects anything that is neither, on read as well as on write.
