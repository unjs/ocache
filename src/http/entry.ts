...
// nothing is storable — not even a lifetime to advertise, since the early return below is
// taken before any of it — and the live response flows back out through `index.ts` exactly
// as a bypassed one does (`value instanceof Response`).
...
// `//`, not JSDoc: an internal helper, and the `raw` bundle budget counts JSDoc blocks.
