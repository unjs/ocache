...
- **Multi-byte typed arrays render element values, not bytes.** Raw bytes would make an `Int32Array` key depend on machine endianness, so `BYTES_PER_ELEMENT > 1` keeps the decimal `join`.
- **Bytes render as length-prefixed base64.** `ArrayBuffer`, `SharedArrayBuffer`, `DataView`, and the byte-sized views (`BYTES_PER_ELEMENT === 1`) have no element reading distinct from their bytes, so byte order does not enter the rendering and the compact form is available. Decimal element values cost about four characters and one string allocation **per byte**, which a binary argument pays on the key path before its cached function has run once; base64 is 4/3 of the bytes through the runtime's own encoder (`src/base64.ts`, whose arms differ by 45x — see `.agents/http/response.md`). `BYTES_PER_ELEMENT` is what selects the branch: `"join" in value` used to, which happened to route `DataView` correctly and would silently move a byte-sized view back onto the slow path. The tag still separates the types, so a `Uint8Array` and an `Int8Array` over the same bytes never share a key.
