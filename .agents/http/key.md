...
`_path` is `pathname` plus the search that `HandlerConfig.allowedQueryNames` allows, which is empty by default: the query is opt-in through `allowQuery`, and only `allowQuery: true` leaves `_url.search` unfiltered. `key.ts` and `request.ts` must read that one resolved field so the key and the handler's URL carry the same search; see `.agents/http/request.md`.
...
`_pathname` is `escapeKey(decodeURI(_url.pathname)).slice(0, 16)`, or `"index"` when that is empty and `"-"` when `decodeURI` throws on a malformed escape. It reads `_url.pathname` **directly**. It used to parse a second `URL` over `pathname + search` to strip the query, which cost a parse and an allocation on every miss and every hit. `pathname` is already the parsed path and cannot contain a raw `?`, so re-parsing it produced the same string. `escapeKey` then removes every non-word character, which absorbs the one remaining difference: the re-parse prefixed an opaque path with `/`, and `/` is stripped. Verified against 193 462 http(s) URLs, including percent escapes, dot segments, backslashes, and non-ASCII: zero differing prefixes. The only input whose prefix moved is an **opaque** path carrying dot segments (`x-opaque:a/../b`), which the re-parse resolved and reading `pathname` does not. Its hash component is unchanged, so keys stay unique; only the debug prefix reads differently.

Keep the prefix cheap and keep it out of identity. It exists so a human can read a stored key. Any change to how it renders moves **every** stored key, so `test/index.test.ts` pins the exact bytes for a handful of paths. Do not remove `decodeURI` to save the try/catch: it is worth 0.16 µs and would rewrite the prefix of every percent-encoded path (`/blog/hello%20world` reads `bloghello20world` rather than `bloghelloworld`).

`resolveKey` returns `_hashedPath` on its own when the handler declares no keyed header and no cookie allowlist, which is the default. Joining a one-element array yields that same string, so this is the same key, without the two intermediate arrays.
